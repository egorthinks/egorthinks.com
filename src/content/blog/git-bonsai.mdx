...
import Leaderboard from '../../components/ui/Leaderboard.astro';
import SideNote from '../../components/ui/SideNote.astro';
import SpecList from '../../components/ui/SpecList.astro';

<Frame variant="ticks" label="at a glance" class="not-prose mt-2 mb-10 px-5 py-6 sm:px-6">
    <SpecList
        items={[
            { term: 'What', value: 'A bonsai grown from your commit history' },
            { term: 'Install', value: 'npx git-bonsai' },
            { term: 'Playground', value: 'Grow one in the browser', href: '/bonsai/' },
            { term: 'Output', value: 'SVG, PNG, wind loop, growth timelapse' },
            { term: 'Gallery', value: 'Bonsai Hall of Fame', href: 'https://github.com/egorthinks/git-bonsai/blob/main/HALL_OF_FAME.md' },
            { term: 'Source', value: 'egorthinks/git-bonsai', href: 'https://github.com/egorthinks/git-bonsai' }
        ]}
    />
</Frame>
...
<Frame label="playground" class="not-prose my-10 px-5 py-6 sm:px-6 sm:py-7">
    <span class="eyebrow">Go and meet yours</span>
    <p class="mt-3 font-serif text-xl leading-snug font-medium sm:text-2xl">Type a username. Watch the tree grow.</p>
    <p class="text-main/70 mt-2.5 max-w-lg text-sm leading-relaxed">
        The playground runs this exact engine in your browser: seasons, wind, PNG and GIF export. No install, no token, no signup.
    </p>
    <div class="mt-6 flex flex-wrap items-center gap-4">
        <Button href="/bonsai/">Grow a bonsai</Button>
        <span class="text-main/50 text-sm">or try <a class="underline decoration-dashed underline-offset-4 hover:decoration-solid" href="/bonsai/?u=torvalds">torvalds</a></span>
    </div>
</Frame>
...
And that is exactly what a commit history is. Ten years of an account is not a green calendar heatmap, it is a biography with growth spurts, obsessions, long silences and comebacks. git-bonsai just makes the biography visible: your years become growth rings, your streaks bloom, your burnout gaps leave honest deadwood. <Annotate mark="underline">You don't draw this tree, you live it.</Annotate>
...
- **Total contributions** fill the crown. More commits means denser foliage.<SideNote>Private contributions count too, if you enable them on your profile.</SideNote>
...
My favorite detail: the tree lives with the calendar. Leaves flush light in spring, deepen in autumn and frost over in winter, while your languages' hues stay recognizable all year.<SideNote>Only true greens turn amber. A TypeScript-blue crown stays TypeScript-blue.</SideNote>
...
<Artifact
    id="hall-of-fame"
    kind="Gallery"
    title="What the greats grew"
    lede="Three of the sixteen keepers, ranked by the length of the history that fed the tree."
    source="Grown from public GitHub history with the same rules as every other tree, then regenerated from cached metrics so the gallery stays reproducible bit for bit."
    class="my-10"
>
    <Leaderboard
        heads={['#', 'Keeper', 'Tree', 'Years']}
        rows={[
            { name: 'Salvatore Sanfilippo', score: 17, note: 'formal upright, shari scar' },
            { name: 'Linus Torvalds', score: 15, note: 'slanting sumo pine' },
            { name: 'Guido van Rossum', score: 14, note: 'maple' }
        ]}
    />
</Artifact>
...
The fastest way to meet your tree is the **[playground](/bonsai/)**: type a username, watch it grow, download the PNG. It runs the same engine in your browser, over public data only.

For the real thing, add `.github/workflows/bonsai.yml` to your profile repo (the one named after your username):
...
<Callout variant="repro" title="Reproduce this" class="my-8">
    Two lines, no CI, no token needed for the offline mode:
    ...
    Same username and same metrics produce bit-identical bytes, so anything you see above you can regrow yourself.
</Callout>
...
The whole point of a unique tree is showing it off. Not ready to add a workflow? **[Grow one in the browser](/bonsai/)** and post the PNG. If you grow one, post it with **#gitbonsai** or drop a link in the [repo discussions](https://github.com/egorthinks/git-bonsai/discussions). I genuinely want to see what a decade of someone else's commits looks like as a living thing. And if your tree comes out with a *shari* scar down the trunk, wear it proudly. It just means you came back.
